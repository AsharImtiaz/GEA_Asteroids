...
const float	SAUCER_SMALL_SPEED = 50.0f;
const float SMALL_BOMB_FIRE_TIME_INTERVAL = .5f;
...
class EnemySmallSpaceShipBehaviourComponent : public Component
{
...
public:
	virtual ~EnemySmallSpaceShipBehaviourComponent() {}

	virtual void Create(AsteroidsLib* engine, GameObject * go, std::set<GameObject*> * game_objects, ObjectPool<Bomb> * bombs_pool, Player * player_tracker)
	{
		Component::Create(engine, go, game_objects);
		this->bomb_pool = bombs_pool;
		this->tracker = player_tracker;
	}
	...
	virtual void Update(float dt)
	{
		...
		if (rand1 == rand2)
		{
			go->degrees = changeDirection1();
		}
		Move(dt * SAUCER_SMALL_SPEED * game_speed);
		rand3 = rand() % 5;
		rand4 = rand() % 5;
		// time to fire bomb from Enemy Space ship
		if ((CanFire()) && (rand3 == rand4))
		{
			xPos = tracker->horizontalPosition;
			yPos = tracker->verticalPosition;
			...
			{
				bomb->Init(go->horizontalPosition, go->verticalPosition, go->degrees,xPos,yPos);
	...
	bool CanFire()
	{
		...
		if ((engine->getElapsedTime() - time_fire_pressed) < (SMALL_BOMB_FIRE_TIME_INTERVAL / game_speed))
			return false;
...
// the enemy spaceship
class EnemySmallSpaceShip : public GameObject
{
public:
	Mix_Chunk * sound = Mix_LoadWAV("data/Sounds/bangMedium.wav");
	int channel;

	virtual ~EnemySmallSpaceShip() { SDL_Log("EnemySmallSpaceShip::~EnemySmallSpaceShip"); }
	...
	virtual void Receive(Message m)
	{
		if (m == HIT)
		{
			SDL_Log("EnemySmallSpaceShip::Hit!");
